...
public class GatewayserverApplication {
    ...
    public RouteLocator TL_BankRouteConfig(RouteLocatorBuilder routeLocatorBuilder) {
        return routeLocatorBuilder.routes().route(p -> p.path("/tl_bank/accounts/**")
...
//								.addResponseHeader("X-Response-Time", LocalDateTime.now().toString()))
            ...
            p -> p.path("/tl_bank/loans/**").filters(f -> f.rewritePath("/tl_bank/loans/(?<segment>.*)", "/${segment}"))
//								.addResponseHeader("X-Response-Time", LocalDateTime.now().toString()))
//								.retry(retryConfig -> retryConfig.setRetries(2)
//										.setMethods(HttpMethod.GET)
//										.setBackoff(Duration.ofMillis(100), Duration.ofMillis(500), 2,true)))
                .uri("lb://LOANS")).route(
            p -> p.path("/tl_bank/cards/**").filters(f -> f.rewritePath("/tl_bank/cards/(?<segment>.*)", "/${segment}"))
//								.addResponseHeader("X-Response-Time", LocalDateTime.now().toString())
//								.addResponseHeader("X-Response-Time", LocalDateTime.now().toString())
